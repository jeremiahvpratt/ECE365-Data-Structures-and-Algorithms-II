...
class hashTable {
	...
	public:
		...
		//The contructor initializes the hash table.
		//Uses getPrime to choose a prime number at least as large as
		//the specified size for the initial size of the hash table.
		hashTable(int size = 0);		//Completed
		
		//Insert the specified key into the hash table.
		//If an optional pointer is provided, associate that pointer with the key.
		//Returns 0 on success, 1 if key already exists in hash table, 2 if rehash fails.
		int insert(const std::string &key, void *pv = NULL);	//Completed	
		
		//Check if the specified key is in the hash table.
		//If so, return true; otherwise, return false.
		bool contains(const std::string &key);					//Completed 
		...
		//Returns 0 on success, 1 if the key does not exist in the hash table.
	...
	private:
		...
		//isOccupied - if false, this entry is empty, and the other fields are meaningless.
		//isDeleted - if true, this item has been lazily deleted.
		//pv - a pointer related to the key; NULL if no pointer was provided to insert.
		class hashItem{
		...
		int filled;   //Number of occupied items int he table.
